/* Dependency-free SHA-256, adapted from the widely used public-domain
 * reference implementation pattern (Brad Conte style). Read-only utility:
 * hashes bytes, produces no offensive capability. */
...
static void sha256_transform(msx_sha256_ctx_t *ctx, const uint8_t data[]) {
...
void msx_sha256_final(msx_sha256_ctx_t *ctx, uint8_t out_hash[32]) {
    ...
    for (i = 0; i < 4; i++) {
        for (int j = 0; j < 8; j++) {
            out_hash[i + 4 * j] = (uint8_t)((ctx->state[j] >> (24 - i * 8)) & 0x000000ff);
        }
    }
}
...
void msx_sha256_hex(const uint8_t *data, size_t len, char out_hex[65]) {
    ...
    uint8_t hash[32];
    msx_sha256_init(&ctx);
    msx_sha256_update(&ctx, data, len);
